...
/**
 * Demonstrates posting shipments and equipment, showing examples of various
 * posting attributes, including several ways of specifying origin and
 * destination locations.
 * 
 * The postXYZ methods intentionally contain a lot of duplicated boilerplate
 * code. Most consumers will only need to look at 1 or 2 posting methods to find
 * the examples required for a specific use case.
 * 
...
public class PostAssets extends BaseSampleClient {
	...
	/**
	 * Posts a truck, specifying the origin by postal code and the destination by
	 * city/state.
	 * 
	...
	/**
	 * Posts a truck, specifying the origin by named postal code and the destination
	 * as a zone.
	 * 
	 * @throws RemoteException
	 */
	protected void postEquipmentNamedPostalCode2AreaZone(final SessionToken sessionToken) throws RemoteException {
	...
	/**
	 * Posts a truck, specifying the origin by postal code and the destination by
	 * 'named lat lon' (latitude/longitude/city/state).
	 * 
	 * @throws RemoteException
	 */
	protected void postEquipmentPostalCode2NamedLatLon(final SessionToken sessionToken) throws RemoteException {
	...
	/**
	 * Posts a truck, specifying the origin by lat/lon and the destination as 'open'
	 * (anywhere).
	 * 
	...
	protected void postEquipmentLatitudeLongitude2OpenSetOptionalItems(final SessionToken sessionToken)
		...
		System.out.println("Posted equipment from Coordinates to Open; AssetId = " + assetId + " AlarmId = " + alarmId);
	}

	/**
	 * Posts a shipment, setting the origin by postal code and the destination by
	 * city/state.
	 * 
	 * @throws RemoteException
	 */
	protected void postShipmentPostalCode2CityStateProvince(final SessionToken sessionToken) throws RemoteException {
	 ...
	 * Posts a shipment.
	 * <p>
	 * Sets the origin by NamedPostalCode (city and state or province plus postal
	 * code).
	 * <p>
	 ...
	 * Posts a shipment.
	 * <p>
	 * Sets the origin by NamedLatLon (city and state or province plus latitude and
	 * longitude).
	 * <p>
	 * Sets the destination by city and state.
	 * <p>
	 * Sets all optional items: LTL, PostersReferenceId, Count, Dimensions,
	 * Availability, and alarm.
	 * <p>
	 * For dimensions, feet and pounds are more commonly used; cubic feet volume and
	 * pounds weight less commonly used.
	 * 
	 * @throws RemoteException
	 */
	protected void postShipmentNamedLatLon2CityStateSetOptionalItems(final SessionToken sessionToken)
		...
		final String alarmId = result.getPostAssetSuccessData().getAlarmId();
		System.out.println(
				"Posted shipment from NamedLatLon to City/State; AssetId = " + assetId + " AlarmId = " + alarmId);
	}

	/**
	 * Posts a shipment, specifying origin and destination by city/state, and
	 * including posting to the DAT truck-stop network and setting rates.
	 * 
	 * @throws RemoteException
	 */
	protected void postShipmentSetTruckstopsAsClosestSetRates(final SessionToken sessionToken) throws RemoteException {
	...
	public void queryMyAssets(final SessionToken sessionToken) throws RemoteException {
		...
		final LookupAssetOperation operation = lookupRequestDoc.addNewLookupAssetRequest().addNewLookupAssetOperation();
